...
@section('content')
                        ...
                        <div class="col-auto">
                            <div class="position-relative d-inline-block">
                                <div id="avatarContainer" class="position-relative d-inline-block mb-3">
                                    @if($user->avatar_path)
                                        <img id="avatarPreview" src="{{ asset($user->avatar_path) }}" alt="{{ $user->name }}"
                                    ...
                                    @else
                                        ...
                                        @endphp
                                        <div id="avatarPreview" class="rounded-circle d-flex align-items-center justify-content-center text-white mx-auto bg-{{ $color }}"
                                             style="width:120px;height:120px;font-size:3rem;">
                                            {{ mb_strtoupper(mb_substr($user->name, 0, 1)) }}
                                        </div>
                                    @endif
                                </div>
                                <label for="avatar" class="position-absolute bottom-0 end-0 bg-white rounded-circle p-1 shadow-sm cursor-pointer" style="transform:translate(10%,10%);cursor:pointer;">
                        ...
                        <div class="col-md-6 d-flex align-items-center">
                            <div class="form-check">
                                <input type="checkbox" name="is_active_in_group" id="is_active_in_group" value="1"
                                       ...
                                       class="form-check-input @error('is_active_in_group') is-invalid @enderror">
                                <label for="is_active_in_group" class="form-check-label">
                                    Активен в Telegram группе
                                </label>
                                @error('is_active_in_group')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        </div>
    ...
    @endpush
@endsection
                    <form action="{{ route('users.update', $user) }}" method="POST" enctype="multipar
