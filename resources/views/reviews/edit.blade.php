...
@section('content')
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-6">
            <a href="{{ route('reviews.show', $review) }}" class="text-blue-600 hover:text-blue-800">← Назад к отзыву</a>
        </div>

        <div class="bg-white rounded-xl shadow-md overflow-hidden">
            <div class="px-6 py-4 bg-gray-50 border-b border-gray-200">
                <h1 class="text-2xl font-bold text-gray-900">Редактировать отзыв</h1>
            </div>

            <form action="{{ route('reviews.update', $review) }}" method="POST" class="p-6">
                @csrf @method('PUT')
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <div class="flex space-x-2" x-data="{ rating: {{ old('rating', $review->rating ?? 5) }} }">
                            @for($i = 1; $i <= 5; $i++)
                ...
                <div class="mt-6 flex justify-end space-x-3">
                    <a href="{{ route('reviews.show', $review) }}" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-lg">
                        Отмена
                    </a>
